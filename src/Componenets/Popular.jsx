        ...
        <div className="flex flex-col gap-6 w-full">
            ...
            <div className="flex items-center gap-2 overflow-x-auto pb-2 hide-scrollbar">
                ...
                <button
                    ...
                    className={`px-3.5 py-1.5 rounded-full text-xs font-semibold shrink-0 transition-all ${selectedGenre === "all"
                ...
                {Object.entries(genreMap).map(([id, name]) => (
                    <button
                        ...
                        className={`px-3.5 py-1.5 rounded-full text-xs font-semibold shrink-0 transition-all ${selectedGenre === id
                                ? "bg-rose-600 text-white shadow-md shadow-rose-950/30"
                                : "bg-slate-800/80 hover:bg-slate-700 text-slate-300 border border-slate-700/60"
                            }`}
                    >
            ...
            <div className="flex items-center justify-center gap-4 py-8">
                ...
                <span className="px-4 py-2 rounded-xl bg-slate-900 border border-slate-800 text-slate-200 text-sm font-bold shadow-inner">
                    Page <span className="text-rose-400">{page}</span> of {popularMoviesData?.total_pages || 500}
                </span>

                <button
                    disabled={loading || (popularMoviesData && page >= popularMoviesData.total_pages)}
